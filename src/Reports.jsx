...
export default function reportTbl() {
    ...
    return (
        ...
        <div className='table-container w-full overflow-x-auto rounded px-4 py-2'>
            <table className="table-auto w-full border-separate border-spacing-y-1 text-xs  min-w-[1200px]">
                <thead>
                    <tr className="bg-slate-900 text-slate-100">
                        {Object.entries(columns).map(([header], index) => (
                            <th key={index} className="px-2 py-2 text-left text-[15px] font-semibold uppercase tracking-[0.35px] whitespace-nowrap">
                                {header}
                            </th>
                        ))}
                    </tr>
